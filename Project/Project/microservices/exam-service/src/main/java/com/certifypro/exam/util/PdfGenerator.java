package com.certifypro.exam.util;
 ...
 * Candidate and program data are fetched from candidate-service via Feign and
 * passed in as DTOs; window/centre/allocation are local entities.
 */
@Component
public class PdfGenerator {

    public byte[] generateHallTicket(SeatAllocation alloc, CandidateDto candidate,
        ...
        addRow(table, "Candidate", nz(candidate == null ? null : candidate.name()));
        addRow(table, "Programme", nz(program == null ? null : program.programName()));
        addRow(table, "Level", program == null ? "-" : nz(program.level()));
